...
network --bootproto static --ip=172.22.1.82 --netmask=255.255.255.0 --gateway=172.22.1.1 --nameserver=172.22.1.3,172.22.1.4 --mtu=1500 --hostname desktop-01.core.home.gatwards.org --device=1c:1b:0d:e2:c6:14
rootpw --iscrypted $6$63af2vwMbmowO55d$C4CiNrljZAZbJauVxRdt8QKsbxBZcNZ.mOoKNtkJGD39nOcDh6sVtbC6Iv8JM9h0q9LJT1mh//rTvcJzvRKff1
...
%post
logger "Starting anaconda desktop-01.core.home.gatwards.org postinstall"
exec < /dev/tty3 > /dev/tty3
#changing to VT 3 so that we can see whats going on....
/usr/bin/chvt 3
(
...
#  interface
real=`grep -l 1c:1b:0d:e2:c6:14 /sys/class/net/*/{bonding_slave/perm_hwaddr,address} 2>/dev/null | awk -F '/' '// {print $5}' | head -1`
...
if [ -f /usr/bin/dnf ]; then
  ...
  grub2-mkconfig -o /boot/grub2/grub.cfg
      ...
      dracut -f --regenerate-all --kernel-cmdline "ip=172.22.1.82::172.22.1.1:255.255.255.0::$real:none nameserver=172.22.1.3 nameserver=172.22.1.4"
